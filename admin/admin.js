...
function createUser() {
  ...
  users[id] = { name, count: 0, vehicle, message };
  saveUsers();
  renderUsers();
...
function incrementWash(id) {
  users[id].count += 1;
  washHistory.push({ id, date: new Date().toLocaleString() });
...
function showHistory(id) {
  ...
  if (history.length === 0) {
    list.innerHTML = "<li>No wash history.</li>";
  } else {
    history.forEach(h => {
      const li = document.createElement("li");
      li.textContent = h.date;
      list.appendChild(li);
    });
  }
...
function renderTopCar() {
  const carCount = {};
  for (const id in users) {
    const car = users[id].vehicle;
    carCount[car] = (carCount[car] || 0) + users[id].count;
  }
  let top = Object.entries(carCount).sort((a, b) => b[1] - a[1])[0];
  document.getElementById("topCar").innerText = top ? `Top Car: ${top[0]} (${top[1]} washes)` : "";
}
...
function renderLeaderboard() {
  ...
  sorted.slice(0, 5).forEach(([id, user], index) => {
    const li = document.createElement("li");
    li.innerHTML = `#${index + 1} ${user.name} (${user.vehicle}) - ${user.count}`;
